...
- Página de condição do céu (`ceu.html`, hoje só na publicação da UFBA): o quadro bruto da câmera all-sky ao lado da máscara de segmentação que o modelo do laboratório prevê sobre esse mesmo quadro (com controle de opacidade sobre os pixels brutos) e a dispersão do índice de claridade `Kt` contra a fração difusa `Kd = Hd/H`, colorida pelas quatro condições de céu de Escobedo et al. (2009), com alternância de classes, sobreposição opcional de três modelos empíricos ajustados a médias horárias — Marques Filho et al. (2016), função só de `Kt` e desenhado como curva; Lemos et al. (2017) e o BRL de Ridley, Boland e Lauret (2010), que dependem de mais cinco preditores e por isso chegam avaliados ponto a ponto no payload e aparecem como nuvem — e exportação CSV. Lê `labmim-sky-v1` de `dataset.paths.sky`, contrato ainda **provisório**; os dois quadros têm nome fixo e são reescritos no lugar, então a página invalida o cache por `?t=` derivado do horário da captura.
