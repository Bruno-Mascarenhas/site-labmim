...
// Signal-safe scaffolding shared by build-all.mjs, check-publications.mjs and
// check-links.mjs: each switches site/ between publications and must put it back on the
// default afterwards, including on Ctrl-C.
...
// Retries once because a Ctrl-C reaches the whole process group: the first attempt is
// often killed along with the build it replaces, while a build spawned afterwards is not.
...
// Without a handler the default disposition kills the process outright, leaving site/ on
// whichever publication was mid-build. It only runs when the signal lands while no child
// is blocking the event loop; a second signal exits immediately.
